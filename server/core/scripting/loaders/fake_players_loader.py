...
def load_fake_players(lua_engine) -> dict:
    ...
    try:
        ...
        defaults = {
            ...
            "locksmith_seed_queue_target": max(1, _to_int(defaults_raw.get("locksmith_seed_queue_target"), 4)),
            "locksmith_generate_customer_chance": max(0.0, min(1.0, _to_float(defaults_raw.get("locksmith_generate_customer_chance"), 0.0))),
            "locksmith_rogue_min_open_jobs": max(0, _to_int(defaults_raw.get("locksmith_rogue_min_open_jobs"), 2)),
            "locksmith_rogue_backlog_threshold": max(0, _to_int(defaults_raw.get("locksmith_rogue_backlog_threshold"), 6)),
            "locksmith_rogue_claim_chance": max(0.0, min(1.0, _to_float(defaults_raw.get("locksmith_rogue_claim_chance"), 0.55))),
